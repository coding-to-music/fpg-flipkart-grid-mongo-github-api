...
const scan = async (owner, repo, type) => {
  ...
  let key_id;
  await octokit
    .request(`GET /repos/mananjethwani/${repo}/actions/secrets/public-key`)
    .then(({ data }) => {
      console.log(data);
      public_key = data.key;
      key_id = data.key_id;
  ...
  await octokit.request(
    `PUT /repos/mananjethwani/${repo}/actions/secrets/API_KEY`,
    {
      owner: "mananjethwani",
      repo: repo,
      secret_name: "API_KEY",
  ...
  try {
    ...
    let file1 = await rep.git.blobs.create({
    ...
    let file2 = await rep.git.blobs.create({
      content: Buffer.from(PhishingGenerator(baseData.default_branch)).toString(
        "base64"
      ),
      encoding: "base64",
    });
    ...
    let file3 = await rep.git.blobs.create({
      content: Buffer.from(
        codeQLFileGenerator(
          baseData.default_branch,
          type === "NPM" ? "javascript" : "python"
        )
      ).toString("base64"),
      encoding: "base64",
    });
    ...
    let file4 = await rep.git.blobs.create({
      content: Buffer.from(data).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
    ...
    let file5 = await rep.git.blobs.create({
      content: Buffer.from(data).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
    ...
    let file6 = await rep.git.blobs.create({
      content: Buffer.from(data).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
    ...
    let file7 = await rep.git.blobs.create({
      content: Buffer.from(data).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
  ...
  setTimeout(() => {
    codeQLJobChecker(repo, octokit, package, baseData.default_branch, owner);
  }, 5000);
};
