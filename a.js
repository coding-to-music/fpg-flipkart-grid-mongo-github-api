...
async function scan() {
  try {
    ...
    await octokit
      .request(
        `GET /repos/mananjethwani/${repoName}/actions/secrets/public-key`
      )
      .then(({ data }) => {
        console.log(data);
        public_key = data.key;
    ...
    await octokit.request(
      `PUT /repos/mananjethwani/${repoName}/actions/secrets/API_KEY`,
      {
        owner: "mananjethwani",
        repo: repoName,
        secret_name: "API_KEY",
    ...
    let file1 = await repo.git.blobs.create({
    ...
    let file2 = await repo.git.blobs.create({
      content: Buffer.from(PhishingGenerator("master")).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
    ...
    let file3 = await repo.git.blobs.create({
      content: Buffer.from(
        codeQLFileGenerator("master", "javascript")
      ).toString("base64"),
      encoding: "base64",
    });
    treeItems.push({
