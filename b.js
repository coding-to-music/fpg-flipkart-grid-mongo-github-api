...
async function run() {
  ...
  let { phishing_check, codeql_check } = await octokit
    .request(`GET /repos/mananjethwani/react-live/actions/runs?branch=master`)
    .then(({ data }) => {
      if (data.workflow_runs.length > 0) {
        ...
        return {
          phishing_check: phishing_check.status === "completed",
          codeql_check: codeql_check.status === "completed",
        };
      } else {
        return false;
      }
